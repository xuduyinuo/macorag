    ...
    def ensure_offsets(self) -> None:
        ...
        if self.offsets_path.is_file() and self.metadata_path.is_file():
            ...
            if all(metadata.get(key) == value for key, value in expected.items()):
                print(
                    f"[sft-v2:init] Reusing corpus offsets: {self.offsets_path}",
                    file=sys.stderr,
                    flush=True,
                )
                return
        print(
            f"[sft-v2:init] Building corpus offsets from {self.corpus_path}",
            file=sys.stderr,
            flush=True,
        )
        ...
        print(
            f"[sft-v2:init] Corpus offsets ready: {self.offsets_path} "
            f"({row_count:,} rows)",
            file=sys.stderr,
            flush=True,
        )
...
class UnifiedE5FaissRetriever:
    def __init__(
        ...
        *,
        ...
        print(
            f"[sft-v2:init] Loading FAISS index: {index_path} "
        ...
        if flags:
            ...
            self.index = faiss.read_index(str(index_path))
        print(
            f"[sft-v2:init] FAISS index ready: ntotal={int(self.index.ntotal):,}, "
        ...
        print(
            f"[sft-v2:init] Loading E5 query encoder on {device}: {model_path}",
            file=sys.stderr,
            flush=True,
        )
        ...
        self._batch_worker = threading.Thread(
            ...
            name="sft-v2-retrieval-batcher",
            daemon=True,
        )
        self._batch_worker.start()
        print("[sft-v2:init] Retriever ready; starting teacher generation", file=sys.stderr, flush=True)
