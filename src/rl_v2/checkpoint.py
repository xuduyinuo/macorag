...
def _copy_prompt_contract(path: Path, config: Any) -> None:
    ...
    payload.update({
        "mappo_prompt_template_version": getattr(
            config, "prompt_template_version", "macorag-policy-v3"
        ),
        "prompt_config_path": str(Path(getattr(
            config, "prompt_config_path", "src/rl_v2/policy_prompts.yml"
        )).resolve()),
        "evidence_pointer_format": "P{local_passage_id}",
        "evidence_rationale_present": False,
