...
from .protocol import role_messages
...
def evidence_action_layout(
    ...
    *,
    ...
    fixed JSON tokens stay diagnostic-only.
    """
    ids = sorted(set(int(item) for item in passage_ids))
    ...
    selection_match = re.search(
        r'"selected_passage_ids":(\[(?:"P\d+"(?:,"P\d+")*)?\])(?=\})',
        response,
    )
    ...
    # header token can merge with the following JSON suffix. Construct the
    ...
    for position, actual in enumerate(chosen_tokens):
        ...
        # is resolved. Do not constrain or optimize its fixed suffix.
        active = [
...
def evidence_completion_budget(
    tokenizer: Any, *, passage_ids: list[int], max_completion_length: int,
) -> dict[str, int]:
    """Measure the longest valid pointer-only Evidence action."""
    selected = ",".join(f'"P{item}"' for item in sorted(set(passage_ids)))
    response = '<update-evidence>{"selected_passage_ids":[' + selected + ']}</update-evidence>'
    counts = {"pointer_only": len(tokenizer.encode(response, add_special_tokens=False))}
...
class RoleConditionedActor:
    ...
    def encode_prompt(self, role: AgentRole, prompt: str) -> list[int]:
        final_round = role is AgentRole.ANSWER and "(final round)" in prompt
        messages = role_messages(role, prompt, final_round=final_round)
        ...
        # Preserve the complete canonical system + few-shot prefix. For the
        # dynamic user state retain both the question/header at the beginning
        # and the newest evidence/instructions at the end. Tail-only truncation
        # can silently remove the question on long multi-round states.
        prefix_ids = list(self.tokenizer.apply_chat_template(
            messages[:-1], add_generation_prompt=False, tokenize=True,
        ))
        if len(prefix_ids) >= self.max_prompt_length:
            raise ValueError(
                "max_prompt_length cannot hold the macorag-policy-v3 few-shot prefix"
            )
        dynamic_budget = self.max_prompt_length - len(prefix_ids)
        if ids[:len(prefix_ids)] != prefix_ids:
            # Defensive fallback for chat templates whose generation suffix
            # rewrites the message boundary.
            return [*prefix_ids, *ids[-dynamic_budget:]]
        dynamic_ids = ids[len(prefix_ids):]
        marker_ids = list(self.tokenizer.encode(
            "\n...[middle of dynamic state truncated]...\n",
            add_special_tokens=False,
        ))
        if len(marker_ids) >= dynamic_budget:
            return [*prefix_ids, *dynamic_ids[-dynamic_budget:]]
        content_budget = dynamic_budget - len(marker_ids)
        head_length = min(256, max(1, content_budget // 3))
        tail_length = content_budget - head_length
        return [
            *prefix_ids,
            *dynamic_ids[:head_length],
            *marker_ids,
            *dynamic_ids[-tail_length:],
        ]
...
class VLLMRoleConditionedActor(RoleConditionedActor):
    ...
    def __init__(self, *args: Any, vllm_client: Any, generation_seed: int, **kwargs: Any) -> None:
        ...
        self._generation_counter_lock = threading.Lock()
    ...
    def generate(
        ...
        import torch
        prompt_ids = self.encode_prompt(role, prompt)
        with self._generation_counter_lock:
