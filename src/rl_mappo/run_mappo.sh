...
# Switch away from src/rl_mappo before invoking Python. Otherwise the local
...
values = (
    ...
    str(int(cfg.get("vllm_port", 8002))),
    str(cfg.get("vllm_gpu_indices", "1")),
    str(cfg.get("vllm_served_model_name", "mappo_base")),
    str(cfg.get("vllm_lora_name", "mappo_policy")),
)
print("\n".join(values))
' "${CONFIG_PATH}"
)

if ((${#VLLM_SETTINGS[@]} != 6)); then
...
if [[ "${LAUNCH_DRY_RUN}" == "1" ]]; then
  "${PYTHON_BIN}" -m src.rl_mappo.launch_vllm \
    --config "${CONFIG_PATH}" "${VLLM_ADAPTER_ARGS[@]}" --dry-run
  printf '[mappo-trainer] CUDA_VISIBLE_DEVICES=%s %s -m src.rl_mappo.train_mappo --config %s' \
    "${TRAIN_GPU_INDEX}" "${PYTHON_BIN}" "${CONFIG_PATH}"
  if ((${#TRAIN_ARGS[@]})); then
    printf ' %q' "${TRAIN_ARGS[@]}"
  fi
  printf '\n'
...
printf '[mappo] starting vLLM on physical GPU(s) %s; service output is hidden\n' \
  "${VLLM_GPU_INDICES}"
setsid "${PYTHON_BIN}" -m src.rl_mappo.launch_vllm \
...
printf '[mappo] vLLM ready at http://%s:%s; starting trainer on physical GPU %s\n' \
  "${VLLM_HOST}" "${VLLM_PORT}" "${TRAIN_GPU_INDEX}"
CUDA_VISIBLE_DEVICES="${TRAIN_GPU_INDEX}" "${PYTHON_BIN}" \
  -m src.rl_mappo.train_mappo --config "${CONFIG_PATH}" "${TRAIN_ARGS[@]}"
