...
def build_command(config: dict[str, Any], *, root: Path, adapter_override: str = "") -> tuple[list[str], dict[str, str]]:
    ...
    lora_name = str(config.get("vllm_lora_name", "mappo_policy"))
    command = [
        sys.executable, "-m", "vllm.entrypoints.openai.api_server",
        "--model", str(model),
        "--served-model-name", str(config.get("vllm_served_model_name", "mappo_base")),
        "--host", str(config.get("vllm_host", "127.0.0.1")),
        "--port", str(int(config.get("vllm_port", 8002))),
